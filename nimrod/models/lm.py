...
# %% auto 0
__all__ = ['logger', 'NNLMConfig', 'NNLM', 'perplexity', 'NNLM_X', 'NNBigram']
...
class NNLM(nn.Module):
    def __init__(self,
                ...
                batch_norm:bool = False # add batchnorm on the hidden layer
                ):
        ...
        # optional batchnorm stabilizes hidden activations & speeds up training (c.f. makemore)
        self.bn = nn.BatchNorm1d(n_h) if batch_norm else nn.Identity()
    ...
    def sample(self, prompt:str, vocab:Vocab, max_new_tokens:int=50, temperature:float=1.0):
        self.eval() # use batchnorm running stats (single-token contexts) & disable train-only layers
...
def perplexity(
        logits:torch.Tensor, # (N, n_vocab) unnormalized scores
        targets:torch.Tensor, # (N,) target token ids
        ignore_index:int=-100 # target value to ignore (e.g. padding)
        )->torch.Tensor:
    "Perplexity = exp(mean cross-entropy) of `targets` under `logits`. Lower is better; equals vocab size for a uniform model."
...
class NNLM_X(Classifier, LightningModule):
    ...
    def on_validation_epoch_end(self) -> None:
        # perplexity = exp(epoch-mean cross-entropy); val_loss already tracks mean CE
        super().on_validation_epoch_end()
        self.log("val/perplexity", torch.exp(self.val_loss.compute()), prog_bar=True, sync_dist=True)

    def on_test_epoch_end(self) -> None:
        super().on_test_epoch_end()
        self.log("test/perplexity", torch.exp(self.test_loss.compute()), prog_bar=True, sync_dist=True)
