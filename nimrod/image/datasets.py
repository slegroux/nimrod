...
# %% auto 0
__all__ = ['logger', 'ImageDataset', 'safe_mnist_download', 'MNISTDataset', 'MNISTDataModule']
...
def safe_mnist_download(root='.', train=False, transform=None):
    """
    Safely download MNIST dataset with error handling
    """
    ...
    # List of alternative download mirrors
    mirrors = [
    ...
    # Patch urllib with custom headers
    ...
    for mirror in mirrors:
        try:
            # Attempt to download
            dataset = MNIST(root, train=train, download=True, transform=transform)
            logger.info(f"Successfully downloaded MNIST dataset from {mirror}")
            return dataset
        except Exception as e:
            logger.warning(f"Failed to download from {mirror}: {e}")
            continue

    # If all mirrors fail, return an empty dataset or handle gracefully
    logger.error("Failed to download MNIST dataset from all mirrors")
